...
const typeMappings = () => ({
  address: {
    builder: (x) => new AddressA(x),
    getter: getAddressA
  },
  'address[]': {
    builder: (x) => new ArrayV<AddressA>(x),
    getter: (x) => getArrayV<AddressA>(x),
  },
  string: {
    builder: (x) => new StringA(x),
    getter: getStringA
  },
  uint256: {
    builder: (x) => new NumberA(x),
    getter: getNumberA
  },
  'uint256[]': {
    builder: (x) => new ArrayV<NumberA>(x),
    getter: (x) => getArrayV<NumberA>(x),
  },
  'uint32[]': {
    builder: (x) => new ArrayV<NumberA>(x),
    getter: (x) => getArrayV<NumberA>(x),
    ...
    async function build<T extends Contract>(
      ...
      const fetchers = constructors.map((abi: any) => {
        let nameArg = implicit ? [] : [
          new Arg('name', getStringA, { default: new StringA(contractName) })
        ];
