...
export async function getANN(
  world: World,
  annexArg: Event
): Promise<ANN> {
  return getWorldContract(world, [['ANN', 'address']]);
}

export async function getANNData(
  world: World,
  annexArg: string
): Promise<[ANN, string, Map<string, string>]> {
  let contract = await getANN(world, <Event>(<any>annexArg));
  let data = getContractData(world, [['ANN', annexArg]]);

  return [contract, annexArg, <Map<string, string>>(<any>data)];
}
...
export async function getXAIData(
  ...
  let contract = await getXAI(world, <Event>(<any>annexArg));
  let data = getContractData(world, [['XAI', annexArg]]);
