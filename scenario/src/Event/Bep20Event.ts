...
async function approve(world: World, from: string, bep20: Bep20, address: string, amount: NumberA): Promise<World> {
  let invokation = await invoke(world, bep20.methods.approve(address, amount.encode()), from, ATokenErrorReporter);
...
async function faucet(world: World, from: string, bep20: Bep20, address: string, amount: NumberA): Promise<World> {
  let invokation = await invoke(world, bep20.methods.allocateTo(address, amount.encode()), from, ATokenErrorReporter);
...
async function transfer(world: World, from: string, bep20: Bep20, address: string, amount: NumberA): Promise<World> {
  let invokation = await invoke(world, bep20.methods.transfer(address, amount.encode()), from, ATokenErrorReporter);
...
async function transferFrom(world: World, from: string, bep20: Bep20, owner: string, spender: string, amount: NumberA): Promise<World> {
  let invokation = await invoke(world, bep20.methods.transferFrom(owner, spender, amount.encode()), from, ATokenErrorReporter);
...
async function setFail(world: World, from: string, bep20: Bep20, fail: boolean): Promise<World> {
  let invokation = await invoke(world, bep20.methods.setFail(fail), from, ATokenErrorReporter);
...
async function setFee(world: World, from: string, bep20: Bep20, basisPointFee: NumberA, maxFee: NumberA): Promise<World> {
...
export function bep20Commands() {
  return [
    ...
    new View<{bep20Arg: StringA, apiKey: StringA}>(`
        #### Verify
    ...
    new Command<{bep20: Bep20, spender: AddressA, amount: NumberA}>(`
        #### Approve
    ...
    new Command<{bep20: Bep20, recipient: AddressA, amount: NumberA}>(`
        #### Faucet
    ...
    new Command<{bep20: Bep20, owner: AddressA, spender: AddressA, amount: NumberA}>(`
        #### TransferFrom
    ...
    new Command<{bep20: Bep20, fail: BoolA}>(`
        #### SetFail
      ...
      [
        new Arg("bep20", getBep20V),
        new Arg("fail", getBoolA)
      ],
      (world, from, {bep20, fail}) => setFail(world, from, bep20, fail.val),
      {namePos: 1}
    ),
    new Command<{bep20: Bep20, paused: BoolA}>(`
        #### Pause
      ...
      [
        new Arg("bep20", getBep20V),
        new Arg("paused", getBoolA, {default: new BoolA(true)})
      ],
      (world, from, {bep20, paused}) => setPaused(world, from, bep20, paused.val),
      {namePos: 1}
    ),
    new Command<{bep20: Bep20, basisPointFee: NumberA, maxFee: NumberA}>(`
        #### SetFee
