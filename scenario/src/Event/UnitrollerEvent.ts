...
export function unitrollerCommands() {
  return [
    ...
    new View<{ unitroller: Unitroller; apiKey: StringA }>(
      `
        #### Verify
      ...
      `,
      'Verify',
      [new Arg('unitroller', getUnitroller, { implicit: true }), new Arg('apiKey', getStringA)],
      (world, { unitroller, apiKey }) => verifyUnitroller(world, unitroller, apiKey.val)
    ),
    new Command<{ unitroller: Unitroller; pendingAdmin: AddressA }>(
      `
        #### AcceptAdmin
    ...
    new Command<{ unitroller: Unitroller; pendingAdmin: AddressA }>(
      ...
      `,
      'SetPendingAdmin',
      [new Arg('unitroller', getUnitroller, { implicit: true }), new Arg('pendingAdmin', getAddressA)],
