...
export async function getProposalId(world: World, governor: Governor, proposalIdent: Event): Promise<number> {
  if (typeof proposalIdent === 'string' && proposalIdent === 'LastProposal') {
    ...
    try {
      return (await getNumberA(world, proposalIdent)).toNumber();
    } catch (e) {
...
async function getProposal(world: World, governor: Governor, proposalIdent: Event, getter: (govener: Governor, number: encodedNumber) => Promise<Event>): Promise<Event> {
  return await getter(governor, new NumberA(await getProposalId(world, governor, proposalIdent)).encode());
}

async function getProposalState(world: World, governor: Governor, proposalIdent: Event): Promise<StringA> {
  ...
  return new StringA(proposalStateEnums[stateEnum]);
}
...
export function proposalFetchers(governor: Governor) {
  const fields = {
    id: getNumberA,
    proposer: getAddressA,
    eta: getNumberA,
    targets: {
      constructor: getArrayV(getStringA),
      getter: async (governor, proposalId) => (await governor.methods.getActions(proposalId).call())[0]
    },
    values: {
      constructor: getArrayV(getNumberA),
    ...
    startBlock: getNumberA,
    endBlock: getNumberA,
    forVotes: getNumberA,
    againstVotes: getNumberA
  };
  ...
  const otherFetchers = <Fetcher<object, Value>[]>[
    new Fetcher<{ proposalIdent: EventV, voter: AddressA }, BoolA>(`
        #### HasVoted
      ...
      async (world, { proposalIdent, voter }) => {
        ...
        return new BoolA(receipt.hasVoted);
      },
      { namePos: 1 }
    ),
    new Fetcher<{ proposalIdent: EventV, voter: AddressA }, BoolA>(`
      ...
      async (world, { proposalIdent, voter }) => {
        const receipt = await governor.methods.getReceipt(await getProposalId(world, governor, proposalIdent.val), voter.val).call();
        return new BoolA(receipt.support);
      },
      { namePos: 1 }
    ),
    new Fetcher<{ proposalIdent: EventV, voter: AddressA }, NumberA>(`
        #### VotesCast
      ...
      async (world, { proposalIdent, voter }) => {
        const receipt = await governor.methods.getReceipt(await getProposalId(world, governor, proposalIdent.val), voter.val).call();
        return new NumberA(receipt.votes);
      },
      { namePos: 1 }
    ),
    new Fetcher<{ proposalIdent: EventV }, StringA>(`
        #### State
