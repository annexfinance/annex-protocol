...
contract XAIVault is XAIVaultStorage {
    ...
    /**
     * @notice Deposit XAI to XAIVault for ANN allocation
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(accANNPerShare).div(1e18);
    ...
    /**
     * @notice Claim ANN from XAIVault
     */
    ...
    function _withdraw(address account, uint256 _amount) internal {
        ...
        updateVault();
        updateAndPayOutPending(account); // Update balances of account this is not withdrawal but claiming ANN farmed
    ...
    /**
     * @notice View function to see pending ANN on frontend
     * @param _user The user to see pending ANN
     */
    function pendingANN(address _user) public view returns (uint256)
    {
        UserInfo storage user = userInfo[_user];

        return user.amount.mul(accANNPerShare).div(1e18).sub(user.rewardDebt);
    }

    /**
     * @notice Update and pay out pending ANN to user
     * @param account The user to pay out
     */
    function updateAndPayOutPending(address account) internal {
        uint256 pending = pendingANN(account);

        if(pending > 0) {
            safeANNTransfer(account, pending);
        }
    }

    /**
     * @notice Safe ANN transfer function, just in case if rounding error causes pool to not have enough ANN
     * @param _to The address that ANN to be transfered
     * @param _amount The amount that ANN to be transfered
     */
    function safeANNTransfer(address _to, uint256 _amount) internal {
        uint256 annBal = ann.balanceOf(address(this));

        if (_amount > annBal) {
            ann.transfer(_to, annBal);
            annBalance = ann.balanceOf(address(this));
        } else {
            ann.transfer(_to, _amount);
            annBalance = ann.balanceOf(address(this));
        }
    }
    ...
    function updatePendingRewards() public {
        uint256 newRewards = ann.balanceOf(address(this)).sub(annBalance);

        if(newRewards > 0) {
            annBalance = ann.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updateVault() internal {
        ...
        accANNPerShare = accANNPerShare.add(pendingRewards.mul(1e18).div(xaiBalance));
        pendingRewards = 0;
    }
    ...
    function setAnnexInfo(address _ann, address _xai) public onlyAdmin {
        ann = IBEP20(_ann);
        xai = IBEP20(_xai);
