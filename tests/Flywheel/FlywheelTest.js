...
describe('Flywheel', () => {
  ...
  describe('getAnnexMarkets()', () => {
    it('should return the annex markets', async () => {
      for (let mkt of [aLOW, aREP, aZRX]) {
        await send(comptroller, '_setAnnexSpeed', [mkt._address, bnbExp(0.5)]);
      }
  ...
  describe('_setAnnexSpeed()', () => {
    it('should update market index when calling setAnnexSpeed', async () => {
      ...
      await send(comptroller, '_setAnnexSpeed', [mkt._address, bnbExp(1)]);
    ...
    it('should correctly drop a ann market if called by admin', async () => {
      ...
      const tx = await send(comptroller, '_setAnnexSpeed', [aLOW._address, 0]);
      expect(await call(comptroller, 'getAnnexMarkets')).toEqual(
        [aREP, aZRX].map((c) => c._address)
      );
      expect(tx).toHaveLog('AnnexSpeedUpdated', {
    ...
    it('should correctly drop a ann market from middle of array', async () => {
      ...
      await send(comptroller, '_setAnnexSpeed', [aREP._address, 0]);
      expect(await call(comptroller, 'getAnnexMarkets')).toEqual(
    ...
    it('should not drop a ann market unless called by admin', async () => {
      ...
      await expect(
        send(comptroller, '_setAnnexSpeed', [aLOW._address, 0], {from: a1})
  ...
  describe('updateAnnexBorrowIndex()', () => {
    it('should calculate ann borrower index correctly', async () => {
      ...
      */
      ...
      const {index, block} = await call(comptroller, 'annexBorrowState', [mkt._address]);
    ...
    it('should not revert or update annexBorrowState index if aToken not in Annex markets', async () => {
      ...
      expect(block).toEqualNumber(100);
      const speed = await call(comptroller, 'annexSpeeds', [mkt._address]);
    ...
    it('should not update index if annex speed is 0', async () => {
      ...
      await send(comptroller, '_setAnnexSpeed', [mkt._address, bnbExp(0.5)]);
      await send(comptroller, 'setBlockNumber', [100]);
      await send(comptroller, '_setAnnexSpeed', [mkt._address, bnbExp(0)]);
      await send(comptroller, 'harnessUpdateAnnexBorrowIndex', [
        mkt._address,
        bnbExp(1.1),
      ]);
  ...
  describe('updateAnnexSupplyIndex()', () => {
    ...
    it('should not update index on non-Annex markets', async () => {
      ...
      await send(comptroller, 'setBlockNumber', [100]);
      await send(comptroller, 'harnessUpdateAnnexSupplyIndex', [
        mkt._address
      ]);
    ...
    it('should not matter if the index is updated multiple times', async () => {
      ...
      await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
  ...
  describe('distributeBorrowerAnnex()', () => {
    ...
    it('should update borrow index checkpoint but not annexAccrued for first time user', async () => {
      const mkt = aREP;
      await send(comptroller, "setAnnexBorrowState", [mkt._address, bnbDouble(6), 10]);
      await send(comptroller, "setAnnexBorrowerIndex", [mkt._address, root, bnbUnsigned(0)]);

      await send(comptroller, "harnessDistributeBorrowerAnnex", [mkt._address, root, bnbExp(1.1)]);
      expect(await call(comptroller, "annexAccrued", [root])).toEqualNumber(0);
      expect(await call(comptroller, "annexBorrowerIndex", [ mkt._address, root])).toEqualNumber(6e36);
    });
    ...
    it('should transfer ann and update borrow index checkpoint correctly for repeat time user', async () => {
      ...
      await send(comptroller, "setAnnexBorrowState", [mkt._address, bnbDouble(6), 10]);
      await send(comptroller, "setAnnexBorrowerIndex", [mkt._address, a1, bnbDouble(1)]);
      ...
      */
      const tx = await send(comptroller, "harnessDistributeBorrowerAnnex", [mkt._address, a1, bnbUnsigned(1.1e18)]);
    ...
    it('should not transfer ann automatically', async () => {
      ...
      await send(comptroller, "setAnnexBorrowState", [mkt._address, bnbDouble(1.0019), 10]);
      await send(comptroller, "setAnnexBorrowerIndex", [mkt._address, a1, bnbDouble(1)]);
      /*
        borrowerAmount = borrowBalance * 1e18 / borrow idx
      ...
      */
      await send(comptroller, "harnessDistributeBorrowerAnnex", [mkt._address, a1, bnbExp(1.1)]);
    ...
    it('should not revert or distribute when called with non-Annex market', async () => {
      ...
      expect(await annBalance(comptroller, a1)).toEqualNumber(0);
      expect(await call(comptroller, 'annexBorrowerIndex', [mkt._address, a1])).toEqualNumber(0);
    });
  });
  ...
  describe('distributeSupplierAnnex()', () => {
    it('should transfer ann and update supply index correctly for first time user', async () => {
      ...
      await send(comptroller, "setAnnexSupplyState", [mkt._address, bnbDouble(6), 10]);
      /*
      ...
      */
      ...
      const tx = await send(comptroller, "harnessDistributeAllSupplierAnnex", [mkt._address, a1]);
      expect(await annexAccrued(comptroller, a1)).toEqualNumber(0);
    ...
    it('should update ann accrued and supply index for repeat user', async () => {
      ...
      await send(mkt, "harnessSetBalance", [a1, bnbUnsigned(5e18)]);
      await send(comptroller, "setAnnexSupplyState", [mkt._address, bnbDouble(6), 10]);
      await send(comptroller, "setAnnexSupplierIndex", [mkt._address, a1, bnbDouble(2)])
      /*
        supplierAmount  = 5e18
        deltaIndex      = marketStoredIndex - userStoredIndex
                        = 6e36 - 2e36 = 4e36
     ...
     await send(comptroller, "harnessDistributeAllSupplierAnnex", [mkt._address, a1]);
      expect(await annexAccrued(comptroller, a1)).toEqualNumber(0);
    ...
    it('should not transfer when annexAccrued below threshold', async () => {
      ...
      await send(comptroller, "setAnnexSupplyState", [mkt._address, bnbDouble(1.0019), 10]);
      /*
        supplierAmount  = 5e17
        deltaIndex      = marketStoredIndex - userStoredIndex
                        = 1.0019e36 - 1e36 = 0.0019e36
      ...
      */
      ...
      await send(comptroller, "harnessDistributeSupplierAnnex", [mkt._address, a1]);
      expect(await annexAccrued(comptroller, a1)).toEqualNumber(0.00095e18);
      expect(await annBalance(comptroller, a1)).toEqualNumber(0);
    });
  ...
  describe('transferANN', () => {
    it('should transfer ann accrued when amount is above threshold', async () => {
      const annexRemaining = 1000, a1AccruedPre = 100, threshold = 1;
      const annBalancePre = await annBalance(comptroller, a1);
      const tx0 = await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
      const tx1 = await send(comptroller, 'setAnnexAccrued', [a1, a1AccruedPre]);
      const tx2 = await send(comptroller, 'harnessTransferAnnex', [a1, a1AccruedPre, threshold]);
    ...
    it('should not transfer when ann accrued is below threshold', async () => {
      const annexRemaining = 1000, a1AccruedPre = 100, threshold = 101;
    ...
    it('should not transfer ann if ann accrued is greater than ann remaining', async () => {
      const annexRemaining = 99, a1AccruedPre = 100, threshold = 1;
      const annBalancePre = await annBalance(comptroller, a1);
      const tx0 = await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
      const tx1 = await send(comptroller, 'setAnnexAccrued', [a1, a1AccruedPre]);
      const tx2 = await send(comptroller, 'harnessTransferAnnex', [a1, a1AccruedPre, threshold]);
  ...
  describe('claimAnnex', () => {
    it('should accrue ann and then transfer ann accrued', async () => {
      const annexRemaining = annexRate.mul(100), mintAmount = bnbUnsigned(12e18), deltaBlocks = 10;
      await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
      await pretendBorrow(aLOW, a1, 1, 1, 100);
      await send(comptroller, '_setAnnexSpeed', [aLOW._address, bnbExp(0.5)]);
      await send(comptroller, 'harnessRefreshAnnexSpeeds');
      const speed = await call(comptroller, 'annexSpeeds', [aLOW._address]);
    ...
    it('should claim when ann accrued is below threshold', async () => {
      const annexRemaining = bnbExp(1), accruedAmt = bnbUnsigned(0.0009e18)
      await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
  ...
  describe('claimAnnex batch', () => {
    it('should revert when claiming ann from non-listed market', async () => {
      const annexRemaining = annexRate.mul(100), deltaBlocks = 10, mintAmount = bnbExp(10);
      await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
      ...
      await expect(send(comptroller, 'claimAnnex', [claimAccts, [aLOW._address, aEVIL._address], true, true])).rejects.toRevert('revert not listed market');
    });
    ...
    it('should claim the expected amount when holders and atokens arg is duplicated', async () => {
      ...
      const tx = await send(comptroller, 'claimAnnex', [[...claimAccts, ...claimAccts], [aLOW._address, aLOW._address], false, true]);
      // ann distributed => 10e18
      for(let acct of claimAccts) {
        expect(await call(comptroller, 'annexSupplierIndex', [aLOW._address, acct])).toEqualNumber(bnbDouble(1.125));
    ...
    it('claims ann for multiple suppliers only', async () => {
      ...
      const tx = await send(comptroller, 'claimAnnex', [claimAccts, [aLOW._address], false, true]);
      // ann distributed => 10e18
      for(let acct of claimAccts) {
        expect(await call(comptroller, 'annexSupplierIndex', [aLOW._address, acct])).toEqualNumber(bnbDouble(1.125));
        expect(await annBalance(comptroller, acct)).toEqualNumber(bnbExp(1.25));
    ...
    it('claims ann for multiple borrowers only, primes uninitiated', async () => {
      const annexRemaining = annexRate.mul(100), deltaBlocks = 10, mintAmount = bnbExp(10), borrowAmt = bnbExp(1), borrowIdx = bnbExp(1)
      await send(comptroller.ann, 'transfer', [comptroller._address, annexRemaining], {from: root});
      let [_,__, ...claimAccts] = saddle.accounts;
      ...
      const tx = await send(comptroller, 'claimAnnex', [claimAccts, [aLOW._address], true, false]);
      for(let acct of claimAccts) {
        expect(await call(comptroller, 'annexBorrowerIndex', [aLOW._address, acct])).toEqualNumber(bnbDouble(2.25));
        expect(await call(comptroller, 'annexSupplierIndex', [aLOW._address, acct])).toEqualNumber(0);
      }
    });
  ...
  describe('harnessRefreshAnnexSpeeds', () => {
    ...
    it('should get correct speeds with borrows', async () => {
      ...
      const speed = await call(comptroller, 'annexSpeeds', [aLOW._address]);
      expect(speed).toEqualNumber(annexRate);
      expect(tx).toHaveLog(['AnnexSpeedUpdated', 0], {
        aToken: aLOW._address,
        newSpeed: speed
      });
    });
    ...
    it('should get correct speeds for 2 assets', async () => {
      ...
      await send(comptroller, 'harnessAddAnnexMarkets', [[aLOW._address, aZRX._address]]);
      await send(comptroller, 'harnessRefreshAnnexSpeeds');
      const speed1 = await call(comptroller, 'annexSpeeds', [aLOW._address]);
      const speed2 = await call(comptroller, 'annexSpeeds', [aREP._address]);
      const speed3 = await call(comptroller, 'annexSpeeds', [aZRX._address]);
      expect(speed1).toEqualNumber(annexRate.div(4));
      expect(speed2).toEqualNumber(0);
      expect(speed3).toEqualNumber(annexRate.div(4).mul(3));
    });
  });
  ...
  describe('harnessAddAnnexMarkets', () => {
    it('should correctly add a annex market if called by admin', async () => {
      ...
      const tx1 = await send(comptroller, 'harnessAddAnnexMarkets', [[aLOW._address, aREP._address, aZRX._address]]);
      const tx2 = await send(comptroller, 'harnessAddAnnexMarkets', [[aBAT._address]]);
      const markets = await call(comptroller, 'getAnnexMarkets');
      expect(markets).toEqual([aLOW, aREP, aZRX, aBAT].map((c) => c._address));
      expect(tx2).toHaveLog('AnnexSpeedUpdated', {
    ...
    it('should not write over a markets existing state', async () => {
      ...
      await send(comptroller, "_setAnnexSpeed", [mkt, 0]);
      await send(comptroller, "harnessAddAnnexMarkets", [[mkt]]);
