...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim ann for at once. default 100 (~2.3M gas)
			...
			for (let borrowerChunk of getChunks(borrowersByAToken[aTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'annexBorrowerIndex',[aTokenAddr, borrower])
					}));
...
let claimAnnexBatch = async (borrowersByAToken, opts) => {
	for (let aTokenAddr of Object.keys(borrowersByAToken)) {
		...
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				...
				try {
					let tx = await send(Comptroller, 'claimAnnex', [
						chunk,
						[aTokenAddr],
						true,
						false,
...
(async () => {
	...
	await claimAnnexBatch(unInit, opts);
